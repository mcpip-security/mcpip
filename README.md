...
| `MCPIP_DELEGATION_ENABLED` | `false` | Attenuated session delegation (`POST /v1/delegate` + authorize-path intersection + cascading revocation — `docs/SESSION_DELEGATION_DESIGN.md`). Off ⇒ the surface does not exist (404) and a token carrying `delegation_id` is denied fail-closed rather than silently un-narrowed. |
...
[`docs/integrate/ARCHITECTURE.md`](docs/integrate/ARCHITECTURE.md) for the full curve (serial vs.
...
- [**The data-plane fork**](docs/integrate/ARCHITECTURE.md) — a **decision memo** on whether MCPIP should ever enter the model's prompt/content path (the "oracle inversion" / taint-tracking pillars). Recommendation: hold the "interceptor, not a proxy" line; this is an explicit **owner decision, pending**. Writes no data-plane code and reserves no pointer token.
