...
with a least-privilege role lives in ``docs/INTEGRATIONS.md``.
...
def main() -> int:
    ...
    # fails with a clear message there. For a real-account run see docs/INTEGRATIONS.md.)
    ...
    if not failures:
        ...
        print(f"{DIM}Next: run it against a REAL table with a least-privilege role — docs/INTEGRATIONS.md{RESET}")
        return 0
