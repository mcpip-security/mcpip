...
| `command not found: mcpip` | The CLI (the `mcpip-sdk` package) is not on your `PATH` — `quickstart.sh` builds the gateway's venv but does not install the CLI. | `pip install mcpip-sdk`, or `pipx install mcpip-sdk` for a global command. |
...
```bash
pipx install mcpip-sdk                             # or: brew install --HEAD mcpip/tap/mcpip
